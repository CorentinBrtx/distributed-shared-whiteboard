...
public class FreeLineComponent implements DSComponent {
	...
	public FreeLineComponent(int x, int y, Color color, int thickness) {
	...
	public boolean equals(Object obj) {
		if (obj instanceof FreeLineComponent) {
			return this.id.equals(((FreeLineComponent) obj).id);
		} else {
			return false;
		}
	}
	...
	public void draw(Graphics2D g, String username) {
		...
		if (this.inModification) {
			if (this.username.equals(username)) {
				...
				g2.drawOval((int) this.x1 - 5, (int) this.y1 - 5, 10, 10);
				g2.drawOval((int) this.x1 - 5, (int) this.y2 - 5, 10, 10);
				g2.drawOval((int) this.x2 - 5, (int) this.y1 - 5, 10, 10);
				g2.drawOval((int) this.x2 - 5, (int) this.y2 - 5, 10, 10);

				g2.dispose();
				this.deleteButton.setBounds((int) x2 - 13, (int) y1 - 23, 20, 17);
			} else {
				Graphics2D g2 = (Graphics2D) g.create();
	...
	public void updateCoordinates() {
		this.x1 = this.getBounds().x;
		this.y1 = this.getBounds().y;
		this.x2 = x1 + this.getBounds().width;
		this.y2 = y1 + this.getBounds().height;
	}
	...
	public void editShape(int x, int y) {
		if (this.modificationType == DSComponent.TOP_LEFT && (x < x2) && (y < y2)) {
			path.transform(new AffineTransform((x - x2) / (x1 - x2), 0, 0, (y - y2) / (y1 - y2),
					x2 * (x1 - x) / (x1 - x2), y2 * (y1 - y) / (y1 - y2)));
		} else if (this.modificationType == DSComponent.BOTTOM_RIGHT && (x > x1) && (y > y1)) {
			path.transform(new AffineTransform((x - x1) / (x2 - x1), 0, 0, (y - y1) / (y2 - y1),
					x1 * (x2 - x) / (x2 - x1), y1 * (y2 - y) / (y2 - y1)));
		} else if (this.modificationType == DSComponent.TOP_RIGHT && (x > x1) && (y < y2)) {
			path.transform(new AffineTransform((x - x1) / (x2 - x1), 0, 0, (y - y2) / (y1 - y2),
					x1 * (x2 - x) / (x2 - x1), y2 * (y1 - y) / (y1 - y2)));
		} else if (this.modificationType == DSComponent.BOTTOM_LEFT && (x < x2) && (y > y1)) {
			path.transform(new AffineTransform((x - x2) / (x1 - x2), 0, 0, (y - y1) / (y2 - y1),
					x2 * (x1 - x) / (x1 - x2), y1 * (y2 - y) / (y2 - y1)));
			...
			path.transform(new AffineTransform(1, 0, 0, 1, xMove, yMove));
		}
		this.updateCoordinates();
	}
