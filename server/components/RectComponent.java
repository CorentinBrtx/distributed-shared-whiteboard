...
public class RectComponent extends Rectangle implements DSComponent {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3190176869865473886L;
	private int x1;
	private int y1;
	private int x2;
	private int y2;
	...
	public RectComponent(int x1, int y1, int x2, int y2, Color color, int thickness) {
	...
	public void draw(Graphics2D g, String username) {
		...
		if (this.inModification) {
			if (this.username.equals(username)) {
				...
				Stroke dashed = new BasicStroke(2, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] { 6 },
						0);
				g2.setStroke(dashed);
				g2.draw(this.getBounds());
	...
	public void editShape(int x, int y) {
		if (this.modificationType == DSComponent.TOP_LEFT) {
			this.x1 = x;
			this.y1 = y;
			this.setFrame(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2 - y1));
	...
	public boolean equals(Object obj) {
		if (obj instanceof RectComponent) {
			return this.id.equals(((RectComponent) obj).id);
		} else {
			return false;
		}
	}
